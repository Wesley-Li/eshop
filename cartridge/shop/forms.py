...
from .wnifields import WniChoiceField,WniComplexChoiceField
...
class DiscountForm(forms.ModelForm):
    ...
    def clean_discount_code(self):
        ...
        """
        from .wniexc import DiscountTotalNotEnough
        ...
        if code:
            ...
            except DiscountTotalNotEnough,e:
...
class OrderForm(FormsetForm, DiscountForm):
    ...
    prefer_time=WniChoiceField(label="配送时间",widget=forms.RadioSelect,choices=((u"立即送餐",u"立即送餐(1小时之内)"),(u"预约",u"预约送达时间")),initial=u"立即送餐")
    #pay_type=WniChoiceField(label="支付方式",widget=forms.RadioSelect,choices=((u"货到付款",u"货到付款"),),initial=u"货到付款")
    pay_type=WniChoiceField(label="支付方式",widget=forms.RadioSelect,choices=((u"货到付款",u"货到付款"),(u"微信支付",u"微信支付")),initial=u"货到付款")
    delivery_type=WniComplexChoiceField(label="配送方式",wnichoices=settings.CENTER_POTS['Nanjing'])#,initial=u"efoodin冷链配送")
    ...
    class Meta:
        ...
        fields = ([f.name for f in Order._meta.fields if
                   f.name in ["billing_detail_first_name","billing_detail_phone","billing_detail_email","billing_detail_street","prefer_time","delivery_type","pay_type","shipping_detail_first_name","shipping_detail_phone","shipping_detail_street"] ] +
                   ["additional_instructions", "discount_code"])
        """
        fields = ([f.name for f in Order._meta.fields if
    ...
    def __init__(self, request, step, data=None, initial=None, errors=None):
        ...
        if settings.WITH_WEIXIN_PAY and 'micromessenger' in request.META.get('HTTP_USER_AGENT').lower():
            self.fields["pay_type"].choices = ((u"货到付款",u"货到付款"),(u"微信支付",u"微信支付"))
        else:
    ...
    def preprocess(cls, data):
        ...
        """
        if data.get("same_billing_shipping", "") == "on": #wni: change on to shit
            for field in data:
