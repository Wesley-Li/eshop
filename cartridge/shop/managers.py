...
class DiscountCodeManager(Manager):
    ...
    def get_valid(self, code, cart):
        ...
        try:
            tmp1 = self.active().get(code=code)
        except:
            raise self.model.DoesNotExist
        else:
            if tmp1.min_purchase is not None and tmp1.min_purchase > cart.total_price():
                from .wniexc import DiscountTotalNotEnough
                raise DiscountTotalNotEnough("订单总额满%s才能使用该券"%str(tmp1.min_purchase))
