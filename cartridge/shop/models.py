...
from django.db.models import CharField, Q, Min
...
class Product(Displayable, Priced, RichText, AdminThumbMixin):
    ...
    def get_storage(self):
        #var_count = self.variations.count()
        least_num = self.variations.aggregate(lst_num=Min('num_in_stock'))
        #return ' > 10' if var_count>=10 else ' < 10'
        return ' > 10' if least_num['lst_num']>=10 else ' < 10'
...
class Order(SiteRelated):
    ...
    #prefer_time=fields.CharField("配送时间",default="立即配送",max_length=30,choices=(("立即送餐","立即送餐"),("预约","预约")))
...
class Cart(models.Model):
    ...
    @property
    def has_bcake(self):
        return any([True if item.unit_price>100 else False for item in self])
    
    @property
    def cartcounter(self):
        """
        wni: for getting cart quantity
        """
...
class DiscountCode(Discount):
    ...
    validusers = models.ManyToManyField(User, blank=True,
                                        related_name="discountcodes",
                                        verbose_name="有效用户")
