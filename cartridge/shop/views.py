...
from mytheme.forms import MyForm
import random
from weixin.payapi import JsApi_pub
...
def product(request, slug, template="shop/product.html",
    ...
    """
    #print '*'*20+'useragent is %s' % request.META['HTTP_USER_AGENT']
...
def cart(request, template="shop/cart.html",
    ...
    request.session['onekeycode'] = ''.join(random.sample('123456789abcdefghigklmnopqrstuvwxyz',4))
    if request.method == "POST":
        valid = True
...
@never_cache
def cart_bak(request, template="shop/cart.html",
         cart_formset_class=CartItemFormSet,
         discount_form_class=DiscountForm):
    """
    Display cart and handle removing items from the cart.
...
def checkout_steps(request, form_class=OrderForm):
    ...
    if settings.WITH_WEIXIN_PAY:
        ...
        if 'micromessenger' in request.META.get('HTTP_USER_AGENT').lower() and not weixinopenid: #and not weixincode:
            api_pub = JsApi_pub()
            ...
            url = api_pub.createOauthUrlForCode('http://%s'%settings.SITE_DOMAIN+reverse('weixin:usercode'))
            return redirect(url)
...
def invoice(request, order_id, template="shop/order_invoice.html",
    ...
    if request.GET.get("format") == "pdf":
        ...
        #pisa.CreatePDF(html, response,link_callback=fetch_resources)
        pisa.CreatePDF(StringIO.StringIO(html.encode("UTF-8")), response,encoding='UTF-8',link_callback=fetch_resources)
        #from reportlab.pdfgen import canvas
        #p = canvas.Canvas(response)
        #p.drawString(0,0,html)
        #p.showPage()
        #p.save()
        return response
    return render(request, template, context)
