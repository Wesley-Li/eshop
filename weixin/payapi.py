...
class WxPayConf_pub(object):
    ...
    APPSECRET = "7367777959d1a20d71dcf4676d5063b0"
    #受理商ID，身份标识
    MCHID = "1232801102"
    #商户支付密钥Key。审核通过后，在微信发送的邮件中查看
    KEY = "CAMIANT1TKLC2EFOODIN3public4abc1"
    ...
    #=======【证书路径设置】=====================================
    #证书路径,注意应该填写绝对路径
    SSLCERT_PATH = "/root/apiclient_cert.pem"
    SSLKEY_PATH = "/root/apiclient_key.pem"
...
class Common_util_pub(object):
    ...
    def trimString(self, value):
        if value is not None and type(value)!=int and len(value) == 0:
            value = None
        return value
...
class JsApi_pub(Common_util_pub):
    ...
    def createOauthUrlForCode(self, redirectUrl):
        ...
        with open('/tmp/wni.log','a') as f:
            f.write('raw redirect url:'+redirectUrl)
        urlObj["redirect_uri"] = quote(redirectUrl,safe='')
        with open('/tmp/wni.log','a') as f:
            f.write('quoted redirect url:'+quote(redirectUrl,safe=''))
    ...
    def setCode(self, code):
        """设置code"""
        ##wni here
        #url = self.createOauthUrlForCode()
        #data = HttpClient().get(url)
        #self.openid = json.loads(data)["openid"]
        #####
        self.code = code
    ...
    def  getParameters(self):
        """设置jsapi的参数"""
        jsApiObj = {}
        #wni: new version wxpay seems no this guy
...
class UnifiedOrder_pub(Wxpay_client_pub):
    ...
    def getPrepayId(self):
        ...
        import time
        with open('/tmp/wni.log','a') as f:
            f.write('before get parepayid')
            f.flush()
            start = time.time()
            self.postXml()
            self.result = self.xmlToArray(self.response)
            prepay_id = self.result["prepay_id"]
            f.write('get prepayid,takes %s secs'% (time.time()-start))
        return prepay_id
